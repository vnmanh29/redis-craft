...
std::shared_ptr<ParsedResult> ResultMove(ParsedResult* dst) {
  std::shared_ptr<ParsedResult> result(new ParsedResult);
  if (dst) {
    result->type = std::move(dst->type);
    result->db_num = dst->db_num;
    result->idle = dst->idle;
    result->db_size = dst->db_size;
    result->expire_size = dst->expire_size;
    result->freq = dst->freq;
    result->aux_field = std::move(dst->aux_field);
    result->expire_time = dst->expire_time;
    result->key = std::move(dst->key);
    result->kv_value = std::move(dst->kv_value);
    result->set_value = std::move(dst->set_value);
    result->map_value = std::move(dst->map_value);
    result->zset_value = std::move(dst->zset_value);
    result->list_value = std::move(dst->list_value);
  }
  return result;
}
  ...
  while (1) {
    ...
    if (type == kAux) {
      ...
      if (!LoadString(&k).ok() || !LoadString(&v).ok()) {
        ...
        Status::Corruption("parse aux kv error");
      } 
      else {
...
static EntryID GetEntryStreamId(const std::string& id, const EntryID &prev_entry_id) {

  auto prev_timestamp = prev_entry_id.timestamp;
  auto prev_sequence_number = prev_entry_id.sequence_number;
...
int ParsedResult::AddStream(const std::vector<std::string>& data, EntryID& entry_id)
{
  /// validate input
  ...
  if (entry_id.timestamp < 0 || entry_id.sequence_number < 0) {
    return -1;
  }
